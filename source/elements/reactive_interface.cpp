...
{
  if (my_time_integrator_type == time_integrator_type::NONE)
    assemble_my_stdy_globals(in_solver);
  if (my_time_integrator_type == time_integrator_type::BDF1)
    assemble_my_bdf1_globals(in_solver);
  if (my_time_integrator_type == time_integrator_type::TRPZ)
    assemble_my_trpz_globals(in_solver);
}
    ...
    solvers::base_implicit_solver<dim, spacedim> *in_solver)
{
  ...
  if (this->local_equation_is_active[0])
  {
    lu_of_Mat3.compute(
      ...
      1. / dt * A2);
    Eigen::MatrixXd Mat4 =
      1. / 2. * (B1.transpose() * (mu_n * A_inv) * C1 + E1 - c_n * E2);
    //
    for (unsigned i_dof = 0; i_dof < indices_n.size(); ++i_dof)
    {
    ...
    //
    d_rho_vec =
      lu_of_Mat3.solve(Fn + 1. / 2. * B1.transpose() * mu_n * A_inv * Rn);
    d_q_vec = mu_n * A_inv * (-Rn + B1 * d_rho_vec);
    NR_rhs = Ln - C1.transpose() * d_q_vec - E1.transpose() * d_rho_vec;
    in_solver->push_to_rhs_vec(indices_n.data(), NR_rhs, ADD_VALUES);
  }
  ...
  {
    lu_of_Mat3.compute(
      1. / 2. * (B1.transpose() * (mu_p * A_inv) * B1 + D1 - c_p * D2) +
      1. / dt * A2);
    Eigen::MatrixXd Mat4 =
      1. / 2. * (B1.transpose() * (mu_p * A_inv) * C1 + E1 - c_p * E2);
    //
    for (unsigned i_dof = 0; i_dof < indices_p.size(); ++i_dof)
    {
      d_rho_vec = lu_of_Mat3.solve(Mat4.col(i_dof));
    ...
    //
    d_rho_vec =
      lu_of_Mat3.solve(Fp + 1. / 2. * B1.transpose() * mu_p * A_inv * Rp);
    d_q_vec = mu_p * A_inv * (-Rp + B1 * d_rho_vec);
    NR_rhs = Lp - C1.transpose() * d_q_vec - E1.transpose() * d_rho_vec;
    in_solver->push_to_rhs_vec(indices_p.data(), NR_rhs, ADD_VALUES);
  }
  ...
  {
    lu_of_Mat3.compute(
      1. / 2. * (B1.transpose() * (mu_r * A_inv) * B1 + D1 - c_r * D2) +
      1. / dt * A2);
    Eigen::MatrixXd Mat4 =
      1. / 2. * (B1.transpose() * (mu_r * A_inv) * C1 + E1 - c_r * E2);
    //
    for (unsigned i_dof = 0; i_dof < indices_r.size(); ++i_dof)
    {
      d_rho_vec = lu_of_Mat3.solve(Mat4.col(i_dof));
    ...
    //
    d_rho_vec =
      lu_of_Mat3.solve(Fr + 1. / 2. * B1.transpose() * mu_r * A_inv * Rr);
    d_q_vec = mu_r * A_inv * (-Rr + B1 * d_rho_vec);
    NR_rhs = Lr - C1.transpose() * d_q_vec - E1.transpose() * d_rho_vec;
    in_solver->push_to_rhs_vec(indices_r.data(), NR_rhs, ADD_VALUES);
  }
  ...
  {
    lu_of_Mat3.compute(
      1. / 2. * (B1.transpose() * (mu_o * A_inv) * B1 + D1 - c_o * D2) +
      1. / dt * A2);
    Eigen::MatrixXd Mat4 =
      1. / 2. * (B1.transpose() * (mu_o * A_inv) * C1 + E1 - c_o * E2);
    //
    for (unsigned i_dof = 0; i_dof < indices_o.size(); ++i_dof)
    {
      d_rho_vec = lu_of_Mat3.solve(Mat4.col(i_dof));
    ...
    //
    d_rho_vec =
      lu_of_Mat3.solve(Fo + 1. / 2. * B1.transpose() * mu_o * A_inv * Ro);
    d_q_vec = mu_o * A_inv * (-Ro + B1 * d_rho_vec);
    NR_rhs = Lo - C1.transpose() * d_q_vec - E1.transpose() * d_rho_vec;
    in_solver->push_to_rhs_vec(indices_o.data(), NR_rhs, ADD_VALUES);
  }
    ...
    solvers::base_implicit_solver<dim, spacedim> *in_solver)
{
  ...
  if (this->local_equation_is_active[0])
  {
    lu_of_Mat1.compute(1. / dt * A2 + B1.transpose() * (mu_n * A_inv) * B1 +
                       D1 - c_n * D2);
    Eigen::MatrixXd Mat2 =
      (B1.transpose() * (mu_n * A_inv) * C1 + E1 - c_n * E2);
    //
  ...
  if (this->local_equation_is_active[1])
  {
    lu_of_Mat1.compute(1. / dt * A2 + B1.transpose() * (mu_p * A_inv) * B1 +
                       D1 - c_p * D2);
    Eigen::MatrixXd Mat2 =
      (B1.transpose() * (mu_p * A_inv) * C1 + E1 - c_p * E2);
    //
  ...
  if (this->local_equation_is_active[2])
  {
    lu_of_Mat1.compute(1. / dt * A2 + B1.transpose() * (mu_r * A_inv) * B1 +
                       D1 - c_r * D2);
    Eigen::MatrixXd Mat2 =
      (B1.transpose() * (mu_r * A_inv) * C1 + E1 - c_r * E2);
    //
  ...
  if (this->local_equation_is_active[3])
  {
    lu_of_Mat1.compute(1. / dt * A2 + B1.transpose() * (mu_o * A_inv) * B1 +
                       D1 - c_o * D2);
    Eigen::MatrixXd Mat2 =
      (B1.transpose() * (mu_o * A_inv) * C1 + E1 - c_o * E2);
    //
...
template <typename BasisType>
void nargil::reactive_interface<dim, spacedim>::hdg_manager<
  BasisType>::advance_me_in_time()
{
  if (my_time_integrator_type == time_integrator_type::BDF1 ||
      my_time_integrator_type == time_integrator_type::TRPZ)
  {
    rho_n_hat0 = rho_n_hat;
    rho_p_hat0 = rho_p_hat;
    rho_r_hat0 = rho_r_hat;
    rho_o_hat0 = rho_o_hat;
    //
    rho_n_vec0 = rho_n_vec;
    rho_p_vec0 = rho_p_vec;
    rho_r_vec0 = rho_r_vec;
    rho_o_vec0 = rho_o_vec;
    //
    q_n_vec0 = q_n_vec;
    q_p_vec0 = q_p_vec;
    q_r_vec0 = q_r_vec;
  ...
  BasisType>::compute_my_linear_matrices()
{
  ...
  A2 = Eigen::MatrixXd::Zero(n_scalar_unkns, n_scalar_unkns);
...
template <typename BasisType>
void nargil::reactive_interface<dim, spacedim>::hdg_manager<
  BasisType>::compute_my_nonlinear_bdf1_matrices()
{
  unsigned n_trace_unkns = my_basis->n_trace_unkns_per_cell_dof();
  unsigned face_quad_size = my_basis->get_face_quad_size();
  //
  ...
  BasisType>::add_my_nonlinear_terms()
{
  ...
  if (my_time_integrator_type == time_integrator_type::BDF1)
    compute_my_nonlinear_bdf1_matrices();
  if (my_time_integrator_type == time_integrator_type::TRPZ)
...
template <typename BasisType>
void nargil::reactive_interface<dim, spacedim>::hdg_manager<
  BasisType>::advance_in_time(reactive_interface *in_cell)
{
  hdg_manager *own_manager =
    static_cast<hdg_manager *>(in_cell->my_manager.get());
  own_manager->advance_me_in_time();
}
