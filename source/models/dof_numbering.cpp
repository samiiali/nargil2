...
template <int dim, int spacedim>
nargil::implicit_hybridized_dof_numbering<
  dim, spacedim>::implicit_hybridized_dof_numbering()
  ...
  ModelType *my_model)
{
  ...
  {
    auto the_worker =
      static_cast<ModelEq *>(i_cell.get())->template get_worker<worker_type>();
    for (unsigned i_face = 0; i_face < 2 * dim; ++i_face)
    {
      if (the_worker->dofs_ID_in_this_rank[i_face].size() == 0)
      {
        ...
        {
          if (the_worker->BCs[i_face] != boundary_condition::periodic)
          {
            the_worker->assign_local_global_cell_data(
              i_face,
              ...
              0);
            local_dof_num_on_this_rank +=
              the_worker->dof_names_on_faces[i_face].count();
            global_dof_num_on_this_rank +=
              the_worker->dof_names_on_faces[i_face].count();
          }
        }
        else
        {
          ...
          if (cell->dealii_cell->neighbor_is_coarser(i_face))
          {
            ...
            //
            dealii_cell_type &&nb_i1 = cell->dealii_cell->neighbor(i_face);
